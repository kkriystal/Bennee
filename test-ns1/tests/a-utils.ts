...
export function createConvertedToTokenEvent(
  by: Address,
  amount: BigInt,
  convertedAmount: BigInt
): ConvertedToToken {
  let convertedToTokenEvent = changetype<ConvertedToToken>(newMockEvent())

  convertedToTokenEvent.parameters = new Array()

  convertedToTokenEvent.parameters.push(
    new ethereum.EventParam("by", ethereum.Value.fromAddress(by))
  )
  convertedToTokenEvent.parameters.push(
  ...
  convertedToTokenEvent.parameters.push(
    new ethereum.EventParam(
      "convertedAmount",
      ethereum.Value.fromUnsignedBigInt(convertedAmount)
    )
  )

  return convertedToTokenEvent
}
